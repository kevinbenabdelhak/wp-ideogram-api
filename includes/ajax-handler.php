...
function generate_featured_image_action() {
    ...
    if (is_wp_error($response)) {
        wp_send_json_error('Erreur lors de la communication avec l\'API : ' . $response->get_error_message());
    }
    ...
    if ($response_code !== 200) {
        $error_msg = isset($data['message']) ? $data['message'] : 'Erreur API HTTP ' . $response_code;
    ...
    if (isset($data['data'][0]['url'])) {
        ...
        if (!$image_info) {
            @unlink($tmp);
            wp_send_json_error('Impossible d\'analyser l\'image téléchargée.');
        }
        ...
        switch ($image_info['mime']) {
            ...
            default:
                @unlink($tmp);
                wp_send_json_error('Format d\'image non supporté.');
        }

        // Compression de l'image
        $quality_level = intval(get_option('wp_ideogram_quality_level', 5));
        $quality = ($quality_level * 10);
        ...
        if (is_wp_error($media_id)) {
            ...
            wp_send_json_error('Erreur lors de l\'upload dans la médiathèque : ' . $media_id->get_error_message());
        }
        ...
        wp_send_json_success([
            'message' => 'Image téléchargée et mise en avant.',
            'thumbnail_url' => $local_image_url, 
            'media_id' => $media_id 
        ]);
    } else {
        wp_send_json_error('L\'URL de l\'image n\'a pas été retournée par l\'API Ideogram.');
    }
}
